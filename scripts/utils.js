...
exports.defaultMessage = (task, target) =>
	shell.echo(colors.cyan.bold(`-\t${emoji.get('clock9')} [${task}] ${target}...`));
	...
	if (target) {
		shell.echo(
			colors.green.bold(
				`-\t${emoji.get(
					'heavy_check_mark'
				)}  ${title}: [${task}] All ${target} dependencies have been installed successfully!`
			)
		);
	} else {
...
exports.defaultErrorMessageInstruct = target =>
	shell.echo(
		colors.yellow(`
        Try to remove ${colors.bold('node_modules')} dir and ${colors.bold(
			'package-lock.json'
		)} file in ${target} dir and try to run the command again.\n
