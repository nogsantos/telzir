...
describe('SpeakMore', () => {
	...
	it('It calculates when valid number are passed and the timer is less then plan timer', () => {
		const test = new SpeakMore({ origin: 11, destiny: 16, timer: 20, plan: 30 });
		expect(test.calculate(1.9)).toBe(0);
	});

	it('It calculates when valid number are passed and the timer is bigger then plan timer [plan 60]', () => {
		const test = new SpeakMore({ origin: 11, destiny: 17, timer: 80, plan: 60 });
		expect(Number(test.calculate(1.7).toFixed(1))).toBe(37.4);
	});

	it('It calculates when valid number are passed and the timer is bigger then plan timer [plan 120]', () => {
